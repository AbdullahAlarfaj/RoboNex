...
class POSApp:
    ...
    def setup_main_page(self):
        ...
        title = tk.Label(self.main_frame, text="روبوت عربة مارت الذكي", font=("Segoe UI", 24, "bold"), bg="black",
                         fg="white")
        title.pack(pady=20)

        scan_btn = ttk.Button(self.main_frame, text="📷 مسح الباركود", command=self.start_scanning)
        scan_btn.pack(pady=10)

        cart_btn = ttk.Button(self.main_frame, text="🛒 عرض السلة", command=lambda: self.show_frame(self.cart_frame))
    ...
    def setup_camera_page(self):
        ...
        cancel_btn = ttk.Button(self.camera_frame, text="❌ إلغاء", command=self.stop_camera)
    ...
    def setup_cart_page(self):
        ...
        title = tk.Label(self.cart_frame, text="🛒 سلة المشتريات", font=("Segoe UI", 20), bg="black", fg="white")
        title.pack(pady=10)
        ...
        entry_label = tk.Label(entry_frame, text="📥 أدخل الباركود:", font=("Segoe UI", 14), bg="black", fg="white")
        ...
        generate_qr_btn = ttk.Button(buttons_frame, text="🔲 توليد QR للسلة", command=self.generate_qr)
        generate_qr_btn.pack(side=tk.LEFT, padx=10)

        clear_cart_btn = ttk.Button(buttons_frame, text="🗑️ تفريغ السلة", command=self.clear_cart)
        clear_cart_btn.pack(side=tk.LEFT, padx=10)

        back_btn = ttk.Button(buttons_frame, text="⬅️ رجوع", command=lambda: self.show_frame(self.main_frame))
    ...
    def setup_qr_page(self):
        ...
        back_btn = ttk.Button(self.qr_frame, text="⬅️ رجوع", command=lambda: self.show_frame(self.cart_frame))
    ...
    def setup_message_page(self):
        ...
        ok_btn = ttk.Button(self.message_frame, text="حسنًا", command=self.handle_message_ok)
        ok_btn.pack(pady=10)
    ...
    def setup_manual_error_page(self):
        ...
        label = tk.Label(self.manual_error_frame, text="🚫 المنتج المدخل غير موجود في قاعدة البيانات",
                         font=("Segoe UI", 16), bg="black", fg="white", wraplength=400)
        label.pack(pady=20)

        ok_btn = ttk.Button(self.manual_error_frame, text="حسنًا", command=lambda: self.show_frame(self.cart_frame))
        ok_btn.pack(pady=10)
