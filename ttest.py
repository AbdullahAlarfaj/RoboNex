...
from PyQt5.QtCore import Qt, pyqtSignal, QThread, pyqtSlot, QObject, QRunnable, QThreadPool, QTimer, QEvent
...
class CartWindow(QMainWindow):
    ...
    def setup_conversation_controls(self):
        ...
        self.btn_show_menu.setFixedHeight(55)
    ...
    def open_product_menu(self):
        if not hasattr(self, 'db_manager') or not self.db_manager:
            print("⚠️ DB Manager Missing")
            return
        dialog = ProductSelectionDialog(self.db_manager, self)

        # ربط إشارة التحديث
        dialog.product_added_signal.connect(lambda: self.request_fetch.emit())

        # ✅ حفظ النافذة في المتغير لنستطيع إغلاقها لاحقاً
        self.active_menu_dialog = dialog

        # فتح النافذة (سيتوقف الكود هنا حتى تغلق النافذة)
        dialog.exec_()

        # ✅ بعد الإغلاق، نصفر المتغير
        self.active_menu_dialog = None
